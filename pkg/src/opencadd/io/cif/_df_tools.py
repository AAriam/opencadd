...
class CIFDictItemDataFrame(PolarsDataFrame):
    ...
    def match_names(self, col_name: str = "name", reduce: bool = True):
        """
        ...
        """
        df_ = self.df.with_columns(("_" + self._col_cat + "." + self._col_key).alias("__name__"))
        col_is_sub = PolarsDataFrame(df_).column_is_subset(col_1="__name__", col_2=col_name)
        return (
            (col_is_sub[1] is True or col_is_sub[1] == "__name__")
            if reduce
            else col_is_sub[2]["2_is_sub_1"]
        )
    ...
    def break_down_data_name_column(
        ...
        if not (
            "_" + df_new[col_name_key_new] + "." + df_new[col_name_key_new]
            == df_new[col_name_data_name]
        ).all():
            raise ValueError
    ...
    def match_cat_key(self, col_name_category: str = "category", col_name_keyword: str = "keyword"):
        unique_nonmatching_frame_codes = (
            ...
            .select(pl.col(["frame_code_category", "frame_code_keyword"]))
            .unique()
        )

        unique_matching_frame_codes = (
            self.df.join(
                unique_nonmatching_frame_codes,
        ...
        return unique_matching_frame_codes.sort(
            ["frame_code_category", "frame_code_keyword"]
        ).frame_equal(
